...
from pytfex.transformer.routing_net import RouteTensor
...
def test_train():
    ...
    params:
      ...
      layers:
        - num: 3
          type: 'RoutingModelLayer'
          params:
            hidden_dim: {hdn_dim}
            num_nodes: {2 * blk_size}
            num_heads: 5
            k: 5
            dropout: 0.01
        - num: 1
    ...
    for epoch in range(5):
        for x, y_true in dl:
            x = RouteTensor(data=x)
            opt.zero_grad()
            y = model(x)
            # print(y.data, y_true)
        #     loss = loss_fn(y.data, y_true.reshape(-1))
        #     loss.backward()
        #     opt.step()
            break
        break
